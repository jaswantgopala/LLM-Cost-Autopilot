...
class Router:
    ...
    def get_model_for_tier(self, tier: int) -> ModelConfig:
        """Get the primary ModelConfig for a given tier."""
    ...
    def get_fallback_model_for_tier(self, tier: int) -> ModelConfig:
        """Get the fallback ModelConfig for a given tier."""
        tier_key = f"tier_{tier}"
    ...
    def route(self, prompt: str) -> dict:
        """
        Full routing decision for a prompt: predicted tier + chosen model.
        Returns a dict so it's easy to log later.
        """
